...
public class WorldEvent {
	...
	public BlockPos coordSource;
	public BlockPos coordDestination;
	...
	public WorldEvent(int parDim, String parName, EnumWorldEventType parType, BlockPos source, BlockPos dest) {
		this();
		type = parType;
		coordSource = source;
    	...
    	if (lastCheckedInvasionCount == 0 || DimensionManager.getWorld(dimensionID).getWorldTime() % 100 == 0) {
	    	for (int i = 0; i < invasionEntities.size(); i++) {
	    		...
	    		if (ent.isDead) {
	    			...
	    			//HostileWorlds.dbg("murrrrrrrr" + ((EntityLivingBase)ent).getDistance(coordDestination.getX(), coordDestination.getY(), coordDestination.getZ()));
	    		}
	    	}
    ...
    public void invasionStart() {
    	//HostileWorlds.dbg("Invasion started for: " + coordSource.getX() + ", " + coordSource.getY() + ", " + coordSource.getZ());
    	invasionActive = true;
    	//invasionEntities.clear();
    }
    
    public void invasionEnd() {
    	//HostileWorlds.dbg("Invasion ended for: " + coordSource.getX() + ", " + coordSource.getY() + ", " + coordSource.getZ());
    ...
    public void calculatePlayerRatingData() {
		...
		/*TileEntity tEnt = DimensionManager.getWorld(dimensionID).getBlockTileEntity(coordSource.getX(), coordSource.getY(), coordSource.getZ());
    ...
    public void updateCursedPlayersList(boolean clearList) {
		...
		for (int i = 0; i < worldObj.playerEntities.size(); i++) {
			...
			if (!cursedPlayers.contains(CoroUtilEntity.getName(entP)) && entP.getDistance(coordDestination.getX(), coordDestination.getY(), coordDestination.getZ()) < maxDist) {
	...
	public void writeNBT(NBTTagCompound data) {
    	...
    	int dist = -1;
    	if (coordDestination != null && invasionEntities.size() > 0) dist = (int) ((EntityLivingBase)invasionEntities.get(0)).getDistance(coordDestination.getX(), coordDestination.getY(), coordDestination.getZ());
