...
public class WorldDirector implements Runnable {
    ...
    public void addTickingLocation(ISimulationTickable location, boolean init) {
        ...
        if (location.getOrigin() != null) {
            Integer hash = PathPointEx.makeHash(location.getOrigin().getX(), location.getOrigin().getY(), location.getOrigin().getZ());
    ...
    public ISimulationTickable getTickingSimulationByLocation(BlockPos parCoords) {
        Integer hash = PathPointEx.makeHash(parCoords.getX(), parCoords.getY(), parCoords.getZ());
        ...
        if (ConfigCoroUtil.trackPlayerData) {
            if (world.getTotalWorldTime() % PlayerDataGrid.playerTimeSpentUpdateInterval == 0) {
                for (int i = 0; i < world.playerEntities.size(); i++) {
                    ...
                    ChunkDataPoint cdp = Objects.requireNonNull(WorldDirectorManager.instance()).getChunkDataGrid(world).getChunkData(MathHelper.floor(entP.posX) / 16, MathHelper.floor(entP.posZ) / 16);
    ...
    public void readFromNBT(NBTTagCompound parData) {
        ...
        while (it.hasNext()) {
            ...
            if (locationObj != null) {
                ...
                //System.out.println("reading in ticking location: " + nbt.toString() + " - " + entrance.getOrigin().getX() + " - " + entrance.spawn.posZ);
            }
        }
    }
